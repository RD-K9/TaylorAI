...
export async function savePerson(dataUrl, label) {
  ...
  const row = { id: newId(), dataUrl, fingerprint: fp, createdAt: Date.now(), label };
  await withStore('people', 'readwrite', (s) => s.put(row));
  return row;
}

export async function saveGarment({ imageBase64, source, prompt, label }) {
  const fp = fingerprint(imageBase64);
...
export async function removeGarment(id) {
